...
path = "OneDrive - Greater Manchester/APEP/data/"


###----Import prescriptions and clean----
mero_taz_cont <- read_excel(paste0(path, "2025_mero_taz_cont.xlsx"), 
...
mero_taz_cont <- mero_taz_cont |> clean_names() |> 
  drop_na(ward_end_dtm) |> 
  rename(hosp_no = patient_number,
         presc_dtm = requested_dtm)
  
mero_taz_cont <- mero_taz_cont |>
  mutate(
    equiv = str_match(summary_line, "-\\s*([^-]*?)\\s*equivalent")[, 2] |> 
      str_trim()
    ) |> 
...
mero_taz_c_admin_sum <- mero_taz_c_admin |>
  summarise(
...
mero_taz_cont_courses <- mero_taz_c_admin_sum |> 
  left_join(mero_taz_cont, by = join_by(hosp_no, closest(course_start >= presc_dtm))) |> 
...
adm_data = mero_taz_cont_courses |> 
...
#----Writing to files---
write_xlsx(adm_data, path = paste0(path, "c_adm_data.xlsx"))
write_xlsx(mero_taz_cont_courses, path = paste0(path, "cont_courses.xlsx"))
